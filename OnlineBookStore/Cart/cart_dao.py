class CartDAO:
    def __init__(self):
        self.__cart_db = {}  # member_id : {book_no : CartItem}
    ...
    def insert_or_update_item(self, item):
        ...
        if item.book_no in cart:
            cart[item.book_no].quantity += item.quantity
        else:
            cart[item.book_no] = item
        return True

    def delete_item(self, member_id, book_no):
        cart = self.__cart_db.get(member_id, {})
        if str(book_no) not in cart:
            return False
        del cart[str(book_no)]
        return True
