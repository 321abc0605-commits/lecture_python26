...
class MemberDAO:
    def __init__(self):
        self.__member_db = {}
        self.insert_member(Member('admin', '1111', '관리자', 'admin'))
        self.insert_member(Member('test', '1111', '고재혁', 'customer'))
        self.insert_member(Member('seller', '1111', '기본판매자', 'seller'))
    ...
    def select_members_by_role(self, role):
        members = [m for m in self.__member_db.values() if m.get_role() == role]
        return members if members else None
